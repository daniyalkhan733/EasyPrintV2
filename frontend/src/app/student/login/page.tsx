...
const StudentLoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post(API_ENDPOINTS.studentLogin, { username, password });
      if (response.data.success) {
        localStorage.setItem('student-auth', JSON.stringify(response.data));
        router.push('/');
      } else {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-900 text-white">
      ...
      >
        <h1 className="text-3xl font-bold text-center text-blue-400">Student Login</h1>
        ...
        <p className="text-center text-sm text-gray-400">
          Don&apos;t have an account? <Link href="/student/register" className="text-blue-400 hover:underline">Register here</Link>
        </p>
      </motion.div>
    </div>
  );
